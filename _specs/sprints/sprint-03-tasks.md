...
| S3-TASK-002 | Amend CLAUDE.md + AGENTS.md: Application dependency rule per D1; add Clinician to role lists | Codex | S3-TASK-001 | Pending |
...
| S3-TASK-012 | Enum parity tests: reflection-compare names+values of all Contracts mirrors vs Domain enums (place in Domain.Tests or Application.Tests — the test project may reference both) | Codex | S3-TASK-011, S3-TASK-020 | Pending |
| S3-TASK-013 | Module DTOs in Contracts: Identity (UserSummaryDto, CaregiverSummaryDto/DetailDto, CertificationDto), Clients/CarePlans (ClientSummaryDto/DetailDto, CarePlanDto), Scheduling (ShiftSummaryDto/DetailDto, VisitNoteDto, CreateShiftRequest, CheckInRequest), Billing (InvoiceSummaryDto/DetailDto, PaymentDto) — per Contracts Plan below | Claude | S3-TASK-011, S3-TASK-030 shape review | Pending |
...
| S3-TASK-050 | Infrastructure/WebApi auth foundation: JWT issuance + validation, role policies incl. Clinician, deny-by-default fallback policy, secrets from user secrets/env (never committed) | Codex | S3-TASK-036, S3-TASK-020 | Pending |
| S3-TASK-051 | WebApi problem-details middleware mapping exceptions/validation to `ApiProblemDetails`; verify no PHI in exception output or logs | Codex | S3-TASK-050 | Pending |
| S3-TASK-060 | `CarePath.Client` typed client foundation: `ApiClientBase`, `ITokenStore` abstraction, error mapping to `ApiResponse` (references Contracts only) | Claude | S3-TASK-013 stable | Pending |
| S3-TASK-061 | `CarePath.Client.UI` RCL primitives: status badges, validation summary, paging table shell (references Contracts + Client) | Claude | S3-TASK-060 | Pending |
