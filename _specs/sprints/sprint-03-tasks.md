...
| S3-TASK-010 | Normalize Application/Application.Tests scaffolds per D4; stage sln entries; nest under src/tests | Codex | — | Done 2026-07-04 — net9.0 via props, CPM-compliant, Domain+Contracts refs, placeholders removed |
...
| S3-TASK-030 | Application project real scaffold: references Domain + Contracts; FluentValidation registered; folder layout (`Common/`, `Identity/`, `Clients/`, `Scheduling/`, `Billing/`); move FluentValidation PackageVersion out of the "Test" ItemGroup label in Directory.Packages.props | Codex | S3-TASK-010 | Done 2026-07-04 — folder layout as built: `Abstractions/{Auth,Audit}`, `Common/`, `Auth/`, `Scheduling/`; FluentValidation under "Application" label |
| S3-TASK-031 | `ICurrentUserService` (UserId, Roles, IsSystemActor, CorrelationId) + `SystemActors` well-known identities | Codex | S3-TASK-030 | Done 2026-07-04 — as built: `ICurrentUserContext` + `ISystemAuditContext` (`AuditActorType` covers system actor) |
| S3-TASK-032 | `IAuditLogger` (LogReadAsync/LogWriteAsync: UserId, TimestampUtc, Action, EntityType, EntityId, CorrelationId — never PHI values) + PHI read-audit decorator over `IPhiResource`-marked handlers per D3 | Codex | S3-TASK-031 | Done 2026-07-04 — as built: `IPhiAuditLogger` + `PhiAuditEntry` + `AuditAction`; contract tests in `Application.Tests/Audit` |
| S3-TASK-033 | `IResourceAuthorizationService` + role policy constants + `IClientAccessEvaluator` (family/caregiver scoping) + not-found denial semantics + denial audit per D3 | Codex | S3-TASK-031, S3-TASK-032 | Done 2026-07-04 — as built: `IObjectAuthorizationService` + `IPermissionService` + `IRoleAuthorizationService` + `IIdorGuard`/`IdorGuard` + `ApplicationRoles`; grant-backed evaluator implementation deferred to Sprint 4 (S4-TASK-012, decision D-S4-1) |
| S3-TASK-034 | FluentValidation validators for core commands (CreateShift incl. end-after-start, CreateClient, CreateCaregiver, CreateInvoice, RecordPayment) — PHI-free error messages, no attempted values | Codex | S3-TASK-030, S3-TASK-013 | Done 2026-07-04 (scope-adjusted) — `CreateShiftCommandValidator` shipped as the boundary pattern; remaining module validators move with their commands to Sprint 4 (S4-TASK-030/031/035) |
...
| S3-TASK-036 | JWT/Identity service contracts in Application: `ITokenService`, `IIdentityService`, login/register/refresh request-response shapes (Contracts) | Codex | S3-TASK-030 | Done 2026-07-04 — as built: `IJwtTokenService` + `IIdentityService` + `JwtTokenRequest/Result` + `IdentityUserResult`; Infrastructure implementations landed with S3-TASK-050 |
