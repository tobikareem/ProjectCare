...
- **Fail-closed rules need an explicit trigger, action, and non-action** - "Locator can fail closed" is not a rule. Spell out the comparison (tenant `SchemaVersion` vs application-supported version), the response (reject traffic, audit), and what must not happen (do not open the tenant DbContext).
- **Inspect a document's raw bytes before and after editing it** - A stray `111` prefix on the ADR heading survived a full rewrite because the edits targeted section bodies only. Check `head -c` of the first line when touching any spec file.
- **Do not label a whole store non-PHI or cache security decisions** - The first CP-04 draft called the control plane "non-PHI" and allowed a 60-second cache for organization status. ADR 0003 §8.6 corrected both: a Client-role membership reveals a healthcare relationship, so classify identity and membership records field by field; and authorization freshness (status, role, session, security version, epoch) must come from an authoritative read on every protected request. Only public branding may be cached.
- **A wireframe interaction is a requirement** - The SaaS journey wireframe showed an organization switcher while the draft spec put switching out of scope, and its switch action jumped straight into the other agency's dashboard. Map every wireframe step to requirements (spec §11) and fix the wireframe when it contradicts the ADR, rather than leaving the two to disagree.
- **No in-app organization switching** - A person who belongs to several organizations reaches each one only through that organization's own domain and sign-in. Do not add membership-listing endpoints, switcher pages, or "Switch organization" links; the product owner rejected the switcher even with a re-sign-in at the other host. Tenant identity comes from the verified host, never from a user choice inside the app.

## Multi-Database Workflows (CP-04 design review)

- **Design the intermediate state, not just the happy path, when a write spans two databases** - Membership creation and the backfill each touch the control plane and a tenant database with no distributed transaction. The first design created the membership Active and then created the tenant profile, leaving a window where authentication could see an Active membership with no profile. Rule: create in a Provisioning state, do the cross-database work, activate last; and write cross-database bootstraps as idempotent sagas with a per-step "inspect existing state, then continue" guard and an explicit failure matrix. Never describe two single-database transactions as atomic.
